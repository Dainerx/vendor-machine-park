...
public class AutomateDAO {
	...
	public List<Automate> feedAutomates() {
		...
		/*
		 * Article chips = new Article("chips",10,1.5f); Article madlene = new
		 * Article("madlene",20,1.2f); Article biscuits = new Article("biscuits",5,1f);
		 * Article coke = new Article("coffee",30,1.2f); Article fanta = new
		 * Article("fanta",30,1.2f); Article pepsi = new Article("pepsi",30,1.2f);
		 * Article coffee = new Article("coffee",20,0.5f); Article tea = new
		 * Article("tea",20,0.5f); Article hotChocolate = new
		 * Article("hot chocolate",15,1f);
		 */
	...
	public List<Rapport> feedRapports() {
		List<Automate> listAutomates = feedAutomates();
		List<Rapport> listRapports = new ArrayList<>();
		...
		Iterator<Rapport> it1 = listRapports.iterator();
		Iterator<Automate> it2 = listAutomates.iterator();
		while (it1.hasNext() && it2.hasNext()) {
			it1.next().setAutomate(it2.next());
		}
	...
	public List<Rapport> getAllRapports() {
		...
		Transaction transaction = null;
		List<Rapport> listRapports = null;
		try (Session session = HibernateUtil.getSession()) {
			// start a transaction
			transaction = session.beginTransaction();
			// get an user object

			listRapports = session.createQuery("from Rapport").getResultList();
